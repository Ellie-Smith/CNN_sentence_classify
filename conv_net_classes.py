...
class MLPDropout(object):
    ...
    隐层(全连接层)+ logRegressionLayer(softmax层)
    layer_sizes定义了输入输出的size,在这里，输出size也为最终的输出分类数
    """
    ...
    def predict(self, new_data):
        ...
        y_pred = T.argmax(p_y_given_x, axis=1)  #the index of the maximum value along a given axis 如p_y_given_x=[0,0,1,0.5] return_value=2
        return y_pred
...
class LogisticRegression(object):
    """最外层的softmax层 Multi-class Logistic Regression Class
...
class LeNetConvPoolLayer(object):
    """卷积池化层Pool Layer of a convolutional network """

    def __init__(self, rng, input, filter_shape, image_shape, poolsize=(2, 2), non_linear="tanh"):
        """
        LeNetConvPoolLayer(rng, input=layer0_input,image_shape=(batch_size, 1, img_h, img_w),
